...
def pytorch_expert_forward_backward(
    hidden_states: torch.Tensor,
    down_embed: torch.Tensor,
    up_embed: torch.Tensor,
    indices: torch.Tensor,
    ...
    out = pytorch_expert_forward(hidden, down, up, indices, routing)
    loss = out.sum()
    loss.backward()
    return hidden.grad, down.grad, up.grad, routing.grad
...
def triton_expert_forward_backward(
    hidden_states: torch.Tensor,
    down_embed: torch.Tensor,
    up_embed: torch.Tensor,
    indices: torch.Tensor,
    ...
    out = triton_flash_expert_func(hidden, down, up, indices, routing)
    loss = out.sum()
    loss.backward()
    return hidden.grad, down.grad, up.grad, routing.grad
...
def test_expert_forward_backward_throughput(
...
def test_expert_backward_throughput(
    ...
    impls = testing.get_impls(
        pytorch_impl=pytorch_expert_forward_backward,
        triton_impl=triton_expert_forward_backward,
    )
