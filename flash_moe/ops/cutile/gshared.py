...
@ct.kernel
def _silu_and_mul_kernel(A, B, C, TILE_N: ConstInt):
    """
...
def silu_and_mul_kernel(AB: torch.Tensor, C: torch.Tensor):
    ...
    ct.launch(
        ...
        (A, B, C, next_power_of_2(C.shape[-1])),
    )
...
class CutileGSharedFunc(torch.autograd.Function):
    @staticmethod
    @ensure_contiguous
    def forward(
        ctx,
    ...
    def backward(ctx, grad_output: torch.Tensor):
        raise NotImplementedError("CuTileGSharedFunc backward is not implemented yet.")


def cutile_gshared_func(
    hidden_states: torch.Tensor,
    gate_proj: torch.Tensor,
    up_proj: torch.Tensor,
    down_proj: torch.Tensor,
    ...
    return CutileGSharedFunc.apply(
        hidden_states,
        gate_proj,
        up_proj,
        down_proj,
